...
func getBigMapDiff(ids []int64) error {
	...
	bmd, err := ctx.Domains.BigMapDiffs(ids...)
	if err != nil {
...
func initHandlers() {
	bigMapDiffHandlers = append(bigMapDiffHandlers,
		contractHandlers.NewTokenMetadata(ctx.BigMapDiffs, ctx.Blocks, ctx.TokenMetadata, ctx.Storage, ctx.RPC, ctx.SharePath, ctx.Config.IPFSGateways),
	)
	bigMapDiffHandlers = append(bigMapDiffHandlers,
		contractHandlers.NewTZIP(ctx.BigMapDiffs, ctx.Blocks, ctx.Storage, ctx.TZIP, ctx.RPC, ctx.SharePath, ctx.Config.IPFSGateways),
	)
	bigMapDiffHandlers = append(bigMapDiffHandlers,
		contractHandlers.NewTezosDomains(ctx.Storage, ctx.TezosDomainsContracts),
	)
}

func parseBigMapDiff(bmd domains.BigMapDiff) ([]models.Model, error) {
	items := make([]models.Model, 0)

	storageType, err := getStorageType(*bmd.BigMapDiff)
	if err != nil {
		return nil, err
	}
