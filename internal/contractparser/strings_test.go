...
func TestComputeContractHash(t *testing.T) {
	tests := []struct {
		name    string
		code    string
		want    string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := ComputeContractHash(tt.code)
			if (err != nil) != tt.wantErr {
				t.Errorf("computeContractHash() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if got != tt.want {
				t.Errorf("computeContractHash() = %v, want %v", got, tt.want)
			}
		})
	}
}
