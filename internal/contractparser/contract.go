...
func IsDelegateContract(data gjson.Result) bool {
	...
	if !checkStorageIsDelegate(storage) {
		return false
	}
	code := data.Get("code")
	if !checkCodeIsDelegate(code) {
		return false
	}
	return true
}

func checkStorageIsDelegate(storage gjson.Result) bool {
...
func checkCodeIsDelegate(code gjson.Result) bool {
