...
// NewTokenMetadata -
func NewTokenMetadata(bigMapDiffRepo bigmap.DiffRepository, bigMapStateRepo bigmap.StateRepository, blockRepo block.Repository, tm tokenmetadata.Repository, storage models.GeneralRepository, rpcs map[types.Network]noderpc.INode, sharePath string, ipfs []string) *TokenMetadata {
	parsers := make(map[types.Network]tokens.Parser)
	for network, rpc := range rpcs {
		parsers[network] = tokens.NewParser(bigMapDiffRepo, bigMapStateRepo, blockRepo, tm, storage, rpc, sharePath, network, ipfs...)
	}
...
func (t *TokenMetadata) Do(bmd *domains.BigMapDiff, storage *ast.TypedAst) ([]models.Model, error) {
	tokenParser, ok := t.parsers[bmd.BigMap.Network]
	if !ok {
