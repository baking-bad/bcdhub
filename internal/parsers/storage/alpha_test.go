...
func TestAlpha_ParseOrigination(t *testing.T) {
				...
				BigMaps: []*bigmap.BigMap{
					{
						...
						ValueType:    []byte(`{"prim": "pair", "args": [{"prim": "map", "args": [{"prim": "address"},{"prim": "nat"}], "annots": ["%allowances"]},{"prim": "nat", "annots": ["%balance"]}]}`),
						Name:         "ledger",
						CreatedLevel: 1311215,
						CreatedAt:    consts.BeginningOfTime,
					},
				},
				BigMapState: []*bigmap.State{
					{
						...
						LastUpdateTime:  consts.BeginningOfTime,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Len(t, got.BigMaps, len(tt.want.BigMaps))
			...
			for i := range tt.want.BigMaps {
				tt.want.BigMaps[i].ID = got.BigMaps[i].GetID()
			}
			...
			assert.Equal(t, tt.want.BigMaps, got.BigMaps)
