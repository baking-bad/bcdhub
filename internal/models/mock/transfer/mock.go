...
func (m *MockRepository) GetTransfered(network, contract string, tokenID uint64) (float64, error) {
	m.ctrl.T.Helper()
...
func (m *MockRepository) CalcBalances(network, contract string) ([]transfer.Balance, error) {
	m.ctrl.T.Helper()
	ret := m.ctrl.Call(m, "CalcBalances", network, contract)
	ret0, _ := ret[0].([]transfer.Balance)
	ret1, _ := ret[1].(error)
	return ret0, ret1
}
